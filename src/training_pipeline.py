...
def build_target(df: pd.DataFrame, horizon_hours: int = 72, tolerance_hours: int = 2) -> pd.DataFrame:
    ...
    log.info("Built %d-hour-ahead target: %d/%d rows have a matching future label", horizon_hours, len(merged), before)
    return merged
...
def train_random_forest(X_train, y_train, X_test, y_test):
    ...
    model = RandomForestRegressor(n_estimators=600, max_depth=18, min_samples_leaf=2, max_features=0.8, random_state=42, n_jobs=-1)
...
def push_to_model_registry(model_path: Path, scaler_path: Path, metrics_path: Path, shap_path: Path | None, model_name: str) -> None:
    ...
    for path, repo_path in [
        ...
        api.upload_file(path_or_fileobj=str(path), path_in_repo=repo_path, repo_id=HF_MODEL_REPO, repo_type="model")
...
def run(horizon_hours: int = 72, test_size: float = 0.2, dry_run: bool = False, mlp_epochs: int = 100) -> dict:
    ...
    if len(df) < 20: raise RuntimeError("Not enough rows. Run backfill_historical.py first.")

    df = build_target(df, horizon_hours=horizon_hours)
    feature_cols = select_usable_features(df)
    df = df.dropna(subset=[TARGET_COL]) # Notice we no longer drop based on feature nulls!
    ...
    # NEW: Robust Imputation + Scaling Pipeline
    ...
    joblib.dump(scaler, scaler_path) # Automatically saves the pipeline with the imputer!
    ...
    if not dry_run:
        push_to_model_registry(model_path, scaler_path, metrics_path, shap_path if shap_ok else None, best_name)
